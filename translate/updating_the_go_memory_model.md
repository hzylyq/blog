...
在这个建议之后，Go 内存模型定义了一个传统的happens-before-based的竞争读和写。 就像在 Java 和 JavaScript 中一样，Go 中的读取可以观察任何较早但尚未覆盖的写入，或者任何竞争写； 安排只有一个这样的写入以强制获取固定的结果。

然后，内存模型继续定义建立了协程间通信的同步操作。操作是通常的操作，带有一些 Go 特定的风格：

​	如果包 p 导入包 q，则q的 init 函数的完成发生在任何 p 的开始之前。

​	main.main 函数的启动发生在所有 init 函数完成之后。

​	启动新 goroutine 的 go 语句发生在 goroutine 开始执行之前。

​	chan上的发送发生在来自该chan的相应接收完成之前。

​	chan的关闭发生在由于chan关闭而返回零值的接收之前。

​	来自无缓冲chan的接收发生在该通道上的发送完成之前。

​	容量为 C 的chan的第 k 个接收发生在来自该chan的第 k+C 个发送完成之前。

​	对于任何sync.Mutex 或sync.RWMutex 变量l 且n < m，l.Unlock() 的n次调用发生在l.Lock() 的调用m次返回之前。

​	从 once.Do(f) 对 f() 的单个调用发生（返回）在任何对 once.Do(f) 的调用返回之前。

该列表明显省略了sync/atomic以及包sync更新的 API 的任何提及。

内存模型以一些不正确同步的例子结束。 它不包含错误编译的示例。

## [Changes to Go's Memory Model](https://research.swtch.com/gomm#changes_to_gos_memory_model)

2009 年，在我们着手编写 Go 的内存模型时，Java 内存模型进行了新的修订，C/C++11 内存模型正在最终确定。 一些人强烈鼓励我们采用 C/C++11 模型，充分利用其中我们所做的工作。这对我们来说似乎很冒险。 从而，我们决定采用更保守的方法来保证我们所作，随后十年的论文证实了这一决定，这些论文详细介绍了 Java/C/C++ 内存模型系列中非常微妙的问题。 定义足够多的内存模型来指导程序员和编译器作者很重要，但是完全正式地定义一个模型正确，似乎仍然超出了最有才华的研究人员的掌握范围。能够满足Go继续说明有用的最低要求就足够了。

本节列出了我认为我们应该做出的调整。 如前所述，我打开了一个 [GitHub 讨论](https://github.com/golang/go/discussions/47141)来收集反馈。 基于这些反馈，我计划在本月晚些时候准备一份正式的 Go 提案。

### [Document Go's overall approach](https://research.swtch.com/gomm#document_gos_overall_approach)

“不要聪明”的建议很重要，应该保留，但在深入研究happens-before 的细节之前，我们还需要更多地了解 Go 的总体设计。 我已经看到了 Go 方法的多个错误总结，例如声称 Go 的模型是 C/C++ 的“DRF-SC 或 Catch Fire”。 误读是可以理解的：文档没有说明方法是什么，而且它很短（而且材料很微妙），人们看到的是他们期望看到的东西，而不是那里有或没有的东西。

要添加的文本将类似于以下内容：

​		**综述**

​		Go处理内存模型和其他语言有许多相似之处，旨在保持语义简单、易懂和有用。

​		数据竞争被定义为并发对同一内存位置进行写入读取，除非涉及的所有访问都是由sync/atomic包提供的原子访问。如前所述，强烈		建议程序员使用适当的同步来避免数据竞争。在没有数据竞争的情况下，Go 程序的行为就像所有goroutine 都被多路复用到一个处理器上。此属性有时称为 DRF-SC：无数据竞争的程序以顺序一致的方式执行。

​		其他编程语言通常采用两种方法之一来处理包含数据竞争的程序。第一种，以 C 和 C++ 为例，具有数据竞争的程序是无效的：编译		器可能会以任意令人惊讶的方式破坏它们。 第二种，以 Java 和 JavaScript 为例，具有数据竞争的程序定义了语义，限制了竞争的可能影响并使程序更可靠且更易于调试。 Go 的方法介于两者之间。具有数据竞争的程序在实现可能报告竞争并终止程序的意义上是无效的。 但除此之外，具有数据竞争的程序定义了具有有限数量结果的语义，从而使错误的程序更可靠且更易于调试。

本文将明确Go与其他语言的区别，纠正读者之前的期望。

在“Happens Before”部分的结尾，我们还应该说明某些竞争仍然会导致失败。例如：

​		如果读写的值大于一个机器字，他们的行为回合多机器字操作类似，导致顺序不一致。

我们应该加上一点：

​		应当注意多字之间的数据竞争会导致单次写入产生不一致值。当值依赖于内部(指针， 长度)或(指针，类型)pair的顺序一致性时，正		如Go中实现中的接口，map，切片和字符串的情况，竞争写入会导致内存损坏。

这将更加清楚的说明保证对具有数据竞争程序的限制。

// todo

自编写内存模型以来，新的 API 已添加到sync包中。 我们需要将它们添加到内存模型中（[issue #7948](https://github.com/golang/go/issues/7948)）。 值得庆幸的是，这些添加看起来很简单。我相信它们如下。

​		对于sync.Cond：Broadcast或Signal发生在它解除阻塞的任何调用等待返回之前。
​    	对于sync.Map：Load、LoadAndDelete 和LoadOrStore 是读操作。Delete、LoadAndDelete 和 Store 是写操作。LoadOrStore 当它返回loaded set为 		false 时是一个写操作。写入操作发生在任何观察写入效果的读取操作之前。
​    	对于sync.Pool：在调用Get之前调用Put(x) 会返回相同的值x。 类似地，调用New返回 x 发生在调用 Get 返回相同值 x 之前。
...
上述定义说明，当一个特定的内存必须被多个 goroutines 并发访问而没有其他同步时，消除竞争的唯一方法是让所有访问都使用原子操作。 仅仅让部分访问使用原子操作是不够的。 例如，与原子读或写并发的非原子写仍然是一场竞赛，与非原子读或写并发的原子写也是如此。
