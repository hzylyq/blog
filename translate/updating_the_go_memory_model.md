...
​		数据竞争被定义为并发对同一内存位置进行写入读取，除非涉及的所有访问都是由sync/atomic包提供的原子访问。如前所述，强烈		建议程序员使用适当的同步来避免数据竞争。在没有数据竞争的情况下，Go 程序的行为就像所有goroutine 都被多路复用到一个处		理器上。此属性有时称为 DRF-SC：无数据竞争的程序以顺序一致的方式执行。

​		其他编程语言通常采用两种方法之一来处理包含数据竞争的程序。第一种，以 C 和 C++ 为例，具有数据竞争的程序是无效的：编译		器可能会以任意令人惊讶的方式破坏它们。 第二种，以 Java 和 JavaScript 为例，具有数据竞争的程序定义了语义，限制了竞争的可		能影响并使程序更可靠且更易于调试。 Go 的方法介于两者之间。具有数据竞争的程序在实现可能报告竞争并终止程序的意义上是无效的。 但除此之外，具有数据竞争的程序定义了具有有限数量结果的语义，从而使错误的程序更可靠且更易于调试。
